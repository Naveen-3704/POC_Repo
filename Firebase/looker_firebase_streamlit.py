############## My Current Code (with change having auto id but save session not working)######################
...
def get_firebase_connection():
    cred = credentials.Certificate(r"C:\Users\naveen.koyada\Downloads\firebase-key 1.json")  # Update with correct path
...
def get_button_urls(LOOKER_STUDIO_URL):
    ...
    # Remove headless mode since login requires interaction
    # chrome_options.add_argument('--headless')  # Commenting this to allow login interaction
    ...
    try:
        ...
        driver.get("https://accounts.google.com/")  # Open Google login page
        time.sleep(3)

        # Enter email
        email_input = driver.find_element(By.XPATH, "//input[@type='email']")
        email_input.send_keys("nivedita.rathi@leveragelab.com")  # 🔹 Replace with your email
        email_input.send_keys(Keys.RETURN)
        time.sleep(3)

        # Enter password
        password_input = driver.find_element(By.XPATH, "//input[@type='password']")
        password_input.send_keys("Barwaha@12345")  # 🔹 Replace with your password
        password_input.send_keys(Keys.RETURN)
        time.sleep(5)

        # Step 2: Open Looker Studio Dashboard
...
if page_type == "Navigation Document":
    ...
    # Reset manual Dashboard UID if an existing UID is selected
    if dashboard_uid != "Create DashboardUid":
        st.session_state["manual_dashboard_uid"] = ""  # Resetting the manual UID
...
# ------------------ DROPDOWN PAGES ------------------
    ...
    if not dashboards:
        ...
        parent_dashboard = st.selectbox("📌 Select Parent Document", dashboards, index=dashboards.index(st.session_state.page_data.get('parent_dashboard', dashboards[0])))
...
# ------------------ SUB-PAGE INSIDE EXISTING ------------------
    ...
    if not dashboards:
        ...
        parent_dashboard = st.selectbox("📌 Select Parent Page", dashboards, 
                                        index=dashboards.index(st.session_state.page_data.get('parent_dashboard', dashboards[0])))
...
# Update session state when fields are modified
if page_type == "Navigation Document":
    st.session_state.page_data['dashboard_name'] = dashboard_name
    st.session_state.page_data['dashboard_number'] = dashboard_number
    st.session_state.page_data['dashboard_uid'] = dashboard_uid
elif page_type == "Dropdown Folder":
    st.session_state.page_data['parent_dashboard'] = parent_dashboard
    st.session_state.page_data['advertiser'] = advertiser
    st.session_state.page_data['depth'] = depth
    st.session_state.page_data['icon_name'] = icon_name
    st.session_state.page_data['parent_uid'] = parent_uid
    st.session_state.page_data['page_name'] = page_name
    st.session_state.page_data['page_number'] = page_number
    st.session_state.page_data['page_uid'] = page_uid
    st.session_state.page_data['path'] = path
    st.session_state.page_data['vis_name'] = vis_name
    st.session_state.page_data['vis_page_uid'] = vis_page_uid
    st.session_state.page_data['vis_uid'] = vis_uid
elif page_type == "Dashoard Pages":
    st.session_state.page_data[selected_page_number] = {
        'parent_dashboard': parent_dashboard ,
        'advertiser': advertiser,
